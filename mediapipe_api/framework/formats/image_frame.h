...
MP_CAPI(MpReturnCode) mp_ImageFrame__ui_i_i_ui(mediapipe::ImageFormat::Format format, int width, int height, uint32_t alignment_boundary,
                                               mediapipe::ImageFrame** image_frame_out);
MP_CAPI(MpReturnCode) mp_ImageFrame__ui_i_i_i_Pui8_PF(mediapipe::ImageFormat::Format format, int width, int height, int width_step, uint8_t* pixel_data,
...
MP_CAPI(MpReturnCode) mp_ImageFrame__IsAligned__ui(mediapipe::ImageFrame* image_frame, uint32_t alignment_boundary, bool* value_out);
...
MP_CAPI(uint8_t*) mp_ImageFrame__MutablePixelData(mediapipe::ImageFrame* image_frame);
MP_CAPI(MpReturnCode) mp_ImageFrame__CopyToBuffer__Pui8_i(mediapipe::ImageFrame* image_frame, uint8_t* buffer, int buffer_size);
MP_CAPI(MpReturnCode) mp_ImageFrame__CopyToBuffer__Pui16_i(mediapipe::ImageFrame* image_frame, uint16_t* buffer, int buffer_size);
...
MP_CAPI(MpReturnCode) mp__MakeImageFramePacket_At__Pif_ll(mediapipe::ImageFrame* image_frame, int64_t timestampMicrosec, mediapipe::Packet** packet_out);
