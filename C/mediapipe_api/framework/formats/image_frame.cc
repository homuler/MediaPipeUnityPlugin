...
MpReturnCode mp_ImageFrame__ui_i_i_ui(mediapipe::ImageFormat::Format format,
                                      int width,
                                      int height,
...
MpReturnCode mp_ImageFrame__ui_i_i_i_Pui8_PF(mediapipe::ImageFormat::Format format,
                                             int width,
                                             int height,
                                             int width_step,
                                             uint8* pixel_data,
...
void mp_ImageFrame__delete(mediapipe::ImageFrame* image_frame) {
  delete image_frame;
}

bool mp_ImageFrame__IsEmpty(mediapipe::ImageFrame* image_frame) {
...
bool mp_ImageFrame__IsContiguous(mediapipe::ImageFrame* image_frame) {
  return image_frame->IsContiguous();
}
...
mediapipe::ImageFormat::Format mp_ImageFrame__Format(mediapipe::ImageFrame* image_frame) {
  return image_frame->Format();
}

int mp_ImageFrame__Width(mediapipe::ImageFrame* image_frame) {
  return image_frame->Width();
}

int mp_ImageFrame__Height(mediapipe::ImageFrame* image_frame) {
...
int mp_ImageFrame__WidthStep(mediapipe::ImageFrame* image_frame) {
  return image_frame->WidthStep();
}

uint8* mp_ImageFrame__MutablePixelData(mediapipe::ImageFrame* image_frame) {
  return image_frame->MutablePixelData();
}

int mp_ImageFrame__PixelDataSize(mediapipe::ImageFrame* image_frame) {
  return image_frame->PixelDataSize();
}
...
// StatusOr API
void mp_StatusOrImageFrame__delete(StatusOrImageFrame* status_or_image_frame) {
...
MpReturnCode mp_Packet__GetImageFrame(mediapipe::Packet* packet, mediapipe::ImageFrame** value_out) {
  TRY_ALL {
    auto holder = static_cast<const UnsafePacketHolder<mediapipe::ImageFrame>*>(mediapipe::packet_internal::GetHolder(*packet));
    *value_out = holder->Get();
    RETURN_CODE(MpReturnCode::Success);
  } CATCH_ALL
}
