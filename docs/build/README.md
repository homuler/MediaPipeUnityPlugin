...
- You need to reduce the library size by removing unnecessary code
- You want to modify the native code
- You want to build the library with different options than those used in the distribution package

In such cases, please build the package using one of the following methods:

# (Recommended) Using GitHub Actions

This repository includes a GitHub Actions Workflow for building libraries.
Fork this repository and run the build workflow in your forked repository.
...
> :warning: The Hyper-V backend is required (Windows 10 Home is not supported).
